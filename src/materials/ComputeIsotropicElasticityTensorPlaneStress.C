...
registerMooseObject("SolidMechanicsApp", ComputeIsotropicElasticityTensorPlaneStress);
registerMooseObject("SolidMechanicsApp", ADComputeIsotropicElasticityTensorPlaneStress);
...
InputParameters
ComputeIsotropicElasticityTensorPlaneStressTempl<is_ad, T>::validParams()
{
...
template <bool is_ad, typename T>
ComputeIsotropicElasticityTensorPlaneStressTempl<is_ad, T>::
    ComputeIsotropicElasticityTensorPlaneStressTempl(const InputParameters & parameters)
...
template <bool is_ad, typename T>
void
ComputeIsotropicElasticityTensorPlaneStressTempl<is_ad, T>::residualSetup()
{
  Real elas_mod;
  Real poiss_rat;
  ...
  else if (_lambda_set && _youngs_modulus_set)
  {
    ...
    mooseError("Incorrect combination of elastic properties in "
               "ComputeIsotropicElasticityTensorPlaneStress.");

  // Fill elasticity tensor
  const Real prefactor = elas_mod / (1 - std::pow(poiss_rat, 2));
  std::vector<Real> C{1, poiss_rat, 0, 1, 0, 0, 0, 0, (1 - poiss_rat) / 2};
  std::transform(
      std::begin(C), std::end(C), std::begin(C), [&prefactor](auto & c) { return c * prefactor; });
  _Cijkl.fillSymmetric9FromInputVector(C);
}

template <bool is_ad, typename T>
void
ComputeIsotropicElasticityTensorPlaneStressTempl<is_ad, T>::computeQpElasticityTensor()
{
...
template class ComputeIsotropicElasticityTensorPlaneStressTempl<false, RankFourTensor>;
template class ComputeIsotropicElasticityTensorPlaneStressTempl<true, RankFourTensor>;
template class ComputeIsotropicElasticityTensorPlaneStressTempl<false, SymmetricRankFourTensor>;
template class ComputeIsotropicElasticityTensorPlaneStressTempl<true, SymmetricRankFourTensor>;
