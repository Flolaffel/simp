...
registerMooseObject("MooseApp", LinearNodalConstraintFix);

InputParameters
LinearNodalConstraintFix::validParams()
{
  InputParameters params = NodalConstraintFix::validParams();
...
LinearNodalConstraintFix::LinearNodalConstraintFix(const InputParameters & parameters)
  : NodalConstraintFix(parameters),
...
Real
LinearNodalConstraintFix::computeQpResidual(Moose::ConstraintType type)
{
  /**
...
Real
LinearNodalConstraintFix::computeQpJacobian(Moose::ConstraintJacobianType type)
{
  unsigned int primary_size = _primary_node_ids.size();
