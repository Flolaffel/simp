...
registerMooseObject("OptimizationApp", StressResponseQpPMean);

InputParameters
StressResponseQpPMean::validParams()
{
...
StressResponseQpPMean::StressResponseQpPMean(const InputParameters & parameters)
...
StressResponseQpPMean::computeSensitivity()
{
  ...
  for (auto && [id, elem_data] : _elem_data_map)
  {
    ...
    {
      gamma[dof] += dPMdVM(id) * std::pow(elem_data.physical_density, _q) * vector(x);
      bool dof_is_free =
          std::find(std::begin(_free_dofs), std::end(_free_dofs), dof) != std::end(_free_dofs);
      if (dof_is_free)
        gamma_red[dof] += dPMdVM(id) * std::pow(elem_data.physical_density, _q) * vector(x);
      x++;
    }
  }
