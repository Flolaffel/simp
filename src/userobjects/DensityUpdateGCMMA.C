...
registerMooseObject("OptimizationApp", DensityUpdateGCMMA);

InputParameters
DensityUpdateGCMMA::validParams()
{
...
DensityUpdateGCMMA::DensityUpdateGCMMA(const InputParameters & parameters)
...
void
DensityUpdateGCMMA::gatherElementData()
{
...
void
DensityUpdateGCMMA::performMmaLoop()
{
  ...
  Real a0 = 1;
  std::vector<Real> a(m);
  std::vector<Real> c_MMA(m, 10000);
  std::vector<Real> d(m, 1);
  Real raa0eps = 1e-6;
  ...
  Real raa0 = 0;
  std::vector<Real> xmami(n), xmamieps(n), xmamiinv(n), raa(m), raaeps(m);
  for (unsigned int i = 0; i < n; i++)
  {
    xmami[i] = xmax[i] - xmin[i];
    xmamieps[i] = 1e-5 * eeen[i];
    xmami[i] = std::max(xmami[i], xmamieps[i]);
    xmamiinv[i] = eeen[i] / xmami[i];
    raa0 += std::abs(df0dx[i]) * xmami[i];
    for (unsigned int j = 0; j < m; j++)
      raa[j] += std::abs(dfdx[j][i]) * xmami[i];
  }
  raa0 = std::max(raa0eps, (0.1 / n) * raa0);
  for (unsigned int j = 0; j < m; j++)
    raa[j] = std::max(raaeps[j], (0.1 / n) * raa[j]);
  ...
  if (_t_step <= 2)
  {
    for (unsigned int i = 0; i < n; i++)
    {
      low[i] = xval[i] - asyinit * xmami[i];
      upp[i] = xval[i] + asyinit * xmami[i];
    }
  }
  else
  {
    for (unsigned int i = 0; i < n; i++)
    {
      ...
      lowmin = xval[i] - 10 * xmami[i];
      lowmax = xval[i] - 0.01 * xmami[i];
      uppmin = xval[i] + 0.01 * xmami[i];
      uppmax = xval[i] + 10 * xmami[i];
  ...
  std::vector<Real> ux1(n), ux2(n), xl1(n), xl2(n), uxinv(n), xlinv(n), p0(n), q0(n), pq0(n);
  // Real r0 = f0val;
  for (unsigned int i = 0; i < n; i++)
  {
    ux1[i] = upp[i] - xval[i];
...
std::vector<Real>
DensityUpdateGCMMA::MmaSubSolve(unsigned int m,
                                unsigned int n,
                                Real epsimin,
                                std::vector<Real> low,
                                std::vector<Real> upp,
